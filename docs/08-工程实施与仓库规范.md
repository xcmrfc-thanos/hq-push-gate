...
flowchart TD
  ...
  H --> I[APISIX]
  I --> J[PC Web / H5]
  J --> K[Prometheus / Grafana / Loki 验证]
```
...
internal/observability/      # metrics、trace、structured logging
pkg/                         # 允许被其他 Go 服务复用的稳定小包
tests/                       # 集成测试、协议测试和故障测试
```

规则：`domain` 不依赖 Kafka、Redis 和 Web 框架；`application` 通过接口调用适配器；`cmd` 只负责组装，不写业务逻辑。

> 实现现状（审计注记）：观测统一在共享包 `packages/obs`（非每服务 `internal/observability/`）；
> 测试为同目录 `*_test.go`（无独立 `tests/`）；`pkg/` 未启用——共享代码一律进 `packages/`。
> flink 实际为 `job/function/serde` 三包（domain/state/sink 随 T1 指标引擎演进拆出）；
> biz-service 实际为 `interfaces/application/domain/config`（MyBatis/Redis 适配器暂置 application，
> `infrastructure/` 随 U2 拆分）；前端实际为 `app/pages/services/stores`（features/hooks/types 为规划态）。
...
| 阶段 | 启用内容 |
|---|---|
| U0 | Prometheus、Grafana、结构化 stdout 日志、基础 pprof；Loki/Tempo/Pyroscope 可用单实例 |
| U1 | Loki、Alertmanager、trace_id 贯穿 HTTP/Kafka/WS，接入告警和死信面板 |
